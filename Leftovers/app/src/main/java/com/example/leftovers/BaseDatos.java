...
public class BaseDatos extends SQLiteOpenHelper {
   ...
   public void llenarTablaCategorias(String nombre_categoria, byte[] imagen_categoria, int numero_ing, int num_ing_seleccionados){
    ...
    public int numIng(String categoria){

        int numero= 0;
        String sentencia = "select count(*) from "+ TABLA_INGREDIENTES+ " where ing_categoria = '"+categoria+"' ";
        SQLiteDatabase sqLiteDatabase = this.getWritableDatabase();
    ...
    public void actnumIng(String categoria){
        SQLiteDatabase sqLiteDatabase = this.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put("numero_ing_seleccionados", numIng(categoria));
        String[] args={categoria};
        sqLiteDatabase.update(TABLA_CATEGORIAS, values, "nombre_categoria = ?", args);
    }
...
//se usa para agregar o quitar un ingrediente de la lista
   ...
   public ArrayList<GetRecetas> llenarListaRecetasFiltradas(){
        SQLiteDatabase sqLiteDatabase = this.getReadableDatabase();
        ArrayList<GetRecetas> arrayList = new ArrayList<>();
        Cursor cursor = sqLiteDatabase.rawQuery("select * from "+TABLA_RECETAS+" as r " +
                " where  " +
                " ((r.e1 = '' and r.e2 = '') " +
                " or ( EXISTS (SELECT * FROM " + TABLA_ELEMENTOS+ " WHERE (nombre_ele = r.e1) and agregado_ele = 'Agregado' ) " +
                " and  EXISTS (SELECT * FROM " + TABLA_ELEMENTOS+ " WHERE (nombre_ele = r.e2) and agregado_ele = 'Agregado' ) ) " +
                " or ( EXISTS (SELECT * FROM " + TABLA_ELEMENTOS+ " WHERE (nombre_ele = r.e1) and agregado_ele = 'Agregado' )" +
                " and (r.e2 = '')))" +
                " and (( EXISTS (SELECT * FROM "+ TABLA_INGREDIENTES +" WHERE ( r.i1 like '%' || nombre_ing || '%') and agregado = 'Agregado' )" +
                " and    EXISTS (SELECT * FROM "+ TABLA_INGREDIENTES +" WHERE ( r.i2 like '%' || nombre_ing || '%') and agregado = 'Agregado' )" +
                " and    EXISTS (SELECT * FROM "+ TABLA_INGREDIENTES +" WHERE ( r.i3 like '%' || nombre_ing || '%') and agregado = 'Agregado' )" +
                " and    EXISTS (SELECT * FROM "+ TABLA_INGREDIENTES +" WHERE ( r.i4 like '%' || nombre_ing || '%') and agregado = 'Agregado' )" +
                " and    EXISTS (SELECT * FROM "+ TABLA_INGREDIENTES +" WHERE ( r.i5 like '%' || nombre_ing || '%') and agregado = 'Agregado' ) )" +
                " or ( EXISTS (SELECT * FROM "+ TABLA_INGREDIENTES +" WHERE ( r.i1 like '%' || nombre_ing || '%') and agregado = 'Agregado' ) " +
                " and  EXISTS (SELECT * FROM "+ TABLA_INGREDIENTES +" WHERE ( r.i2 like '%' || nombre_ing || '%') and agregado = 'Agregado' ) " +
                " and  EXISTS (SELECT * FROM "+ TABLA_INGREDIENTES +" WHERE ( r.i3 like '%' || nombre_ing || '%') and agregado = 'Agregado' ) " +
                " and  EXISTS (SELECT * FROM "+ TABLA_INGREDIENTES +" WHERE ( r.i4 like '%' || nombre_ing || '%') and agregado = 'Agregado' ) " +
                " and  r.i5 = '' ))"
    ...
    /*public GetRecetas obtenerReceta2(String nombre_receta){
        SQLiteDatabase sqLiteDatabase = this.getReadableDatabase();
        GetRecetas recetas = null;
        Cursor cursor = sqLiteDatabase.rawQuery("SELECT * FROM " + TABLA_RECETAS+ " AS r WHERE EXISTS (SELECT * FROM " + TABLA_ELEMENTOS+ " WHERE nombre_ele = r.i1 )",new String[]{nombre_receta});
        if(cursor.moveToFirst()){
            recetas = new GetRecetas(
                    cursor.getString(cursor.getColumnIndex("r_nom")),
                    cursor.getBlob(cursor.getColumnIndex("r_ima")),
